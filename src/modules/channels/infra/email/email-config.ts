...
export function createEmailService(config: EmailConfig): EmailService {
  ...
  switch (provider) {
    case "stub":
      logger.info("[EmailConfig] Using StubEmailProvider (emails will be logged to console)");
      return new StubEmailProvider();

    case "resend":
      if (!resendApiKey) {
        logger.error("RESEND_API_KEY environment variable is required when EMAIL_PROVIDER is 'resend'");
        throw new Error(
          "RESEND_API_KEY environment variable is required when EMAIL_PROVIDER is 'resend'"
        );
      }
      if (!fromEmail) {
        logger.error("EMAIL_FROM environment variable is required when EMAIL_PROVIDER is 'resend'");
        throw new Error(
          "EMAIL_FROM environment variable is required when EMAIL_PROVIDER is 'resend'"
        );
      }
      logger.info(`[EmailConfig] Using ResendEmailProvider with sender: ${fromEmail}`);
      return new ResendEmailProvider(resendApiKey, fromEmail);

    default:
      logger.error(`Invalid EMAIL_PROVIDER: ${provider}. Must be 'stub' or 'resend'`);
      throw new Error(
...
export function createEmailServiceFromEnv(): EmailService {
  ...
  if (!provider) {
    logger.error("EMAIL_PROVIDER environment variable is required. Must be 'stub' or 'resend'");
    throw new Error(
